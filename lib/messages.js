...
exports.noisePayload = {
  preencode (state, m) {
    ...
    if (m.userData) c.any.preencode(state, m.userData)
  },
  encode (state, m) {
    let flags = 0
    ...
    if (m.userData) c.any.encode(state, m.userData)
  },
  decode (state) {
    const version = c.uint.decode(state)
    ...
    return {
      ...
      userData: (flags & 64) !== 0 ? c.any.decode(state) : null
    }
  }
}
